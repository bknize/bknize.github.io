...
I co-led the Frontend Guild, focusing on architecture, initiatives, and documentation. Additionally, I joined the UX team’s weekly meeting as a liaison between frontend engineering and design/product; it was a good opportunity to dust off my design chops, and they were happy to start the partnership from that shared context.
...
* Aurelia does have a Redux implementation, which I talk about [here](#/case-study/venminder-state-management). What’s relevant to this is that it justifies using rxjs; if an official Aurelia package has it as a dependency, I can add that version of rxjs as a dependency of vm-library without having to argue the case.
...
Instead of a single component, we created a \`vm-*\` component for each constituent tag of an HTML5 \`<table>\`. Each \`vm-*\` does some bootstrapping to set up event listeners with the base vm-table. The ‘config’ layer now lives in:  
\`<vm-thead>\`  
\`<vm-th sortable searchable name="Username"></vm-th>\`  
\`<vm-th sortable searchable name="Email"></vm-th>\`  
\`</vm-thead>\`  


The \`vm-th\` component replaces the \`<th>\`, and has several bindables which represent the config layer: sorting, filtering, searching, sticky, etc. This is also why the mounting lifecycle hooks in the other \`vm-*\` components handshake with the base; those event-and-listener pathways are how complex behaviors like sticky columns are controlled from a boolean in a \`<th>\`.
...
* GridData was a class a developer could instantiate that would manage your GridModel. I talk a little about it [here](#/case-study/venminder-state-management).
...
* In the case of picnicBasket, each Query had a key which mapped to a filtering function. An example query:  
\`{\`  
\`name: “Search”, key: “picnicSearch”,\`  
\`data: { text: “Hello World”, columns: [“name”, “email”] }\`  
\`}\`  
When picnicBasket is fired, it will find that query in the GridModel and only return array elements that have “Hello World” in either the name or email columns. And this is, loosely, how searching worked.
...
* And, best of all, picnicBasket is optional. In the case of grids which simply cannot operate on a finite array, your function could rely on network requests, mapping the skip/take and QueryModels to API parameters. 
...
“Make the easy way the right way” is something I read, once.`;export{e as default};
