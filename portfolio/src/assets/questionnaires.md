...
* Our **component library** is in-house, developed by a team of frontenders in conjunction with the dedicated design/UX team. More on that [here](#/case-study/venminder-design-system).
...
* As a part of vm-library ([here](#/case-study/venminder-design-system)), we have a standard model for tabular data, but I do not want to operate on half a million objects in memory.
...
Most of our users simply were not operating with more than 300 records, so the UX for those users is identical to any data grid preloading all records from the start. But for our one-percenter clients…it’s the same. Their experience is now identical, too. This also meant we can observe state changes as rxjs Observables, slice and map to chart.js interfaces, and update our visualizations based entirely on the post-filtered datasets.


In addition, using Redux exposes the state engine to the Redux devtools.